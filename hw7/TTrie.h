...
class TTrie {
   private:
    ...
    // Recursive function to copy the trie
    TrieNode<DataType>* copyTrie(const TrieNode<DataType>* node);

    // Recursive function to delete the trie
    void deleteTrie(TrieNode<DataType>* node);

    const TTrie<DataType>* getChildTrie(TrieNode<DataType>* node, const DataType& value) const;

   public:
    /**
     * Default Constructor.
     */
    TTrie() : root(new TrieNode<DataType>()) {}
     ...
     */
    bool compareTries(const TrieNode<DataType>* node1, const TrieNode<DataType>* node2) const;
};
