 ...
 *
 * \param[in] filename    A pointer to the name of the file, passed in via command line argument
 ...
 * \retval  0  if file opens
 * \retval  1  if files fails to open
 * \retval -2  if encountering any other read error
 */
 ...
 *
 * \param[in] filename    A pointer to the name of the file to write
 * \param[in] maze        A 2D array of characters that represents the maze's data
 * \param[in] maze_width  A constant integer, which is the maze's width
 * \param[in] maze_height A constant integer, which is the maze's height
 *
 * \retval 0 on success
 * \retval -4 if failed to open the file to write
 * \retval -5 if encountering any other write error
 */
